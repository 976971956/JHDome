...
- (void)viewDidLoad {
    ...
    info = [[UILabel alloc]initWithFrame:CGRectMake(0, 100, 320, 30)];
    
    [info setText:@"正在打开设备"];
    [info setTextAlignment:NSTextAlignmentCenter];
    [self.view addSubview:info];
    backInfo = [[UILabel alloc]initWithFrame:CGRectMake(0, 200, 320, 30)];
    backInfo.backgroundColor = [UIColor yellowColor];
    [backInfo setText:@"中心设备信息："];
    [backInfo setTextAlignment:NSTextAlignmentCenter];
    [self.view addSubview:backInfo];
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    [peripheralManager stopAdvertising];//停止广播
}
...
- (void)peripheralManager:(CBPeripheralManager *)peripheral didReceiveWriteRequests:(NSArray *)requests{
    ...
    if (request.characteristic.properties & CBCharacteristicPropertyWrite) {
        ...
        NSData *data = [NSData dataWithData:c.value];
        Byte *byte = (Byte *)[data bytes];
        NSLog(@"%@",data);
        NSString *str = [[NSString alloc]initWithBytes:[data bytes] length:data.length encoding:NSUTF8StringEncoding];
        
        if ([str isEqualToString:@"开灯"]) {
        ...
        }else if([str isEqualToString:@"关灯"]){
